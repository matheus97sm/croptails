...
func save_node_data() -> void:
	var save_data_nodes = get_tree().get_nodes_in_group("save_data_component")
	
	game_data_resource = SaveGameDataResource.new()
	
	if save_data_nodes == null:
		return
	
	for save_data_node in save_data_nodes:
		if save_data_node is not SaveDataComponent:
			continue
		
		var save_data_resource: Resource = save_data_node._save_data()
...
func load_game() -> void:
	...
	for resource in game_data_resource.save_data_nodes:
		if resource is not Resource and resource is not NodeDataResource:
			return
