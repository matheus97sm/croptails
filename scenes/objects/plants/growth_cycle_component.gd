...
@export_range(5, 365) var days_until_harvert: int = 7
...
func growth_states(starting_day: int, current_day: int) -> void:
	...
	var num_states = DataTypes.GrowthStates.size() - 1
...
func harvest_state(starting_day: int, current_day: int) -> void:
	...
	var days_passed = (current_day - starting_day) % days_until_harvert
	
	if days_passed == days_until_harvert - 1:
